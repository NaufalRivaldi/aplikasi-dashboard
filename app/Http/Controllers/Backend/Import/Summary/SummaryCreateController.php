...
class SummaryCreateController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('import.summary.index')->with('danger', __('label.FAIL_CREATE_MESSAGE'));
        }
        // --------------------------------------------------------------------
    }
    // ------------------------------------------------------------------------
        ...
        try {
            ...
            if(empty($mSummary)){
                $summary = [
                    ...
                    "siswa_baru"        => $siswaBaru ? $siswaBaru->jumlah : 0,
                    "siswa_cuti"        => $siswaCuti ? $siswaCuti->jumlah : 0,
                    "siswa_keluar"      => $siswaInaktif ? $siswaInaktif->jumlah : 0,
                    "status"            => 0,
                    "cabang_id"         => $input['cabang_id'],
                    "user_id"           => Auth::user()->id,
                ];
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('import.summary.index')->with('danger', __('label.FAIL_UPDATE_MESSAGE'));
        }
        // --------------------------------------------------------------------
    }
    // ------------------------------------------------------------------------
